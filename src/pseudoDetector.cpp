...
{
  src_base = imread( "/home/erickzetinator/Dokumente/PDS/src/proyecto_final/reference/mean10p.png", 1 );
...
void CircleThreshold(int, void*)
{
  ...
  for( size_t i = 0; i < circles.size(); i++ )
  {
    ...
    if (find10(coin) >= .3  && maxR/radius < 1.049019608)
    {
      couter10 = couter10 + 1;
      ...
      if (find50c(coin) >= .1 && maxR/radius >= 1.4901960785)
      // if (find50c(coin) >= .1 || abs(maxR/radius - ratio10_50c) <= epsilon)
      {
        ...
        if (maxR/radius >= 1.049019608 && maxR/radius <= 1.2156862745) 
